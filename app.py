...
JOBTITLE_OPTIONS = [
    "Software Engineer", "Data Engineer", "Graphic Designer", "Social Media Assistant",
    "Administrative Assistant", "Office Manager", "Human Resources", "Business Analyst",
    "Administrator", "IT Professional", "Web Developer", "IT Manager", "Account Manager",
    "Accounting Staff", "Teacher", "Customer Support", "Operations Assistant", "Finance Manager"
]
...
def page_upload():
    ...
    if "custom_selected_skills" not in st.session_state:
        st.session_state.custom_selected_skills = []
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            job_title = st.selectbox("Job Title *", JOBTITLE_OPTIONS)
            
            # 1. Category Selection
            selected_category = st.selectbox(
                "Select Skill Category *", 
                options=list(cv_analyzer.SKILL_CATEGORIES.keys()),
                key="cat_select_main"
            )

            # 2. Dynamic Sub-skills
            available_skills = cv_analyzer.SKILL_CATEGORIES[selected_category]

            required_skills = st.multiselect(
                "Select Required Skills *",
                options=available_skills,
                placeholder=f"Choose {selected_category} skills",
                key=f"skills_select_{selected_category}"
            )
        ...
        if required_skills:
            ...
            ui.render_skill_tags(required_skills, kind="neutral")
        ...
        submitted = st.button("Analyze Candidates", use_container_width=True, type="primary")
    ...
    if submitted:
        ...
        if not required_skills:
            errors.append("Please select at least one required skill.")
...
def page_history():
    ...
    for record in records:
        ...
        if is_expanded:
            with st.container():
                ui.render_candidate_card(record)
